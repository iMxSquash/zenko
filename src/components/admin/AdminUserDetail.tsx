...
import { useAdminDeleteUser, useAdminUpdateUser, useAdminUserDetail } from '@/hooks/useAdmin';
import type { AdminUserUpdate } from '@/hooks/useAdmin';
import { cn } from '@/lib/utils';
import type { ForumUserRole } from '@/types';
import { useNavigate } from '@tanstack/react-router';
import { ArrowLeft, Instagram, Linkedin, Stethoscope, Twitter } from 'lucide-react';
...
export function AdminUserDetail({ userId }: AdminUserDetailProps) {
  ...
  // Valeurs courantes : champ local si modifié, sinon valeur du serveur
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex flex-col gap-6">
        {/* Header card */}
        <section className="flex items-center gap-5 rounded-card-lg border border-border bg-surface p-6 shadow-sm">
          {user.avatarUrl ? (
            <img
              src={user.avatarUrl}
              alt=""
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <div className="flex items-center gap-2">
              <p className="text-h3 font-bold text-text-primary truncate">{displayName}</p>
              {user.isAdmin && (
          ...
          {user.role && (
            <div className="shrink-0 flex items-center gap-2 rounded-full bg-background px-4 py-2">
          ...
          <div className="flex flex-wrap gap-3">
            {ROLES.map((r) => (
              <button
                ...
                className={cn(
                  'flex items-center gap-3 rounded-card border px-4 py-3 transition-all text-body-sm font-semibold',
                  currentRole === r.id
        ...
        <section className="rounded-card-lg border border-danger/30 bg-surface p-6 shadow-sm">
          ...
          {user.isAdmin && (
            <p className="mt-2 text-[12px] text-text-muted">
              Retirez d'abord les droits admin (via SQL) avant de supprimer ce compte.
            </p>
          )}
        </section>
      </div>
