...
create policy "admins_select_self_admins" on public.admins for
...
create policy "fiches_admin_all" on public.fiches for all to authenticated using (public.is_admin (auth.uid ()))
with
    check (public.is_admin (auth.uid ()));

-- Forum : un admin peut supprimer n'importe quel thread
create policy "forum_threads_admin_delete" on public.forum_threads for delete to authenticated using (public.is_admin (auth.uid ()));

-- Forum : un admin peut supprimer n'importe quelle réponse
create policy "forum_replies_admin_delete" on public.forum_replies for delete to authenticated using (public.is_admin (auth.uid ()));

-- Profiles : un admin peut lister tous les profils
create policy "profiles_admin_select_all" on public.profiles for
select to authenticated using (public.is_admin (auth.uid ()));

-- Storage bucket avatars : un admin peut uploader des avatars
create policy "avatars_admin_insert" on storage.objects for
insert
    to authenticated
with
    check (
...
create policy "avatars_admin_delete" on storage.objects for delete to authenticated using (
    bucket_id = 'avatars'
    and public.is_admin (auth.uid ())
);